...
const OrderDetail = ({ mobileNav }) => {
    ...
    const { data: current_order, error } = useSWR(`orders/${order_id}`, fetcher)
                    ...
                    <p className={style.order_status_description}>{`Thank you, your paymant has been successfull. Invoice for your order ${order_id} has been generated and sent to your email id ${current_order?.user?.email || "---------------------"}`}</p>
                        <div className={style.order_details__cont}>
                            <div className={style.order_details_tables}>
                                {!loading ? (<table className="ui celled table fixed table-one">
                                    <tfoot>
                                        <tr>
                                            <td>ORDERID</td>
                                ...
                                </table>) : <TextPlaceHolder paragraph={4} className="table-one"/>}
                                {!loading ? (<table  className="ui celled table fixed table-two">
                                    <tbody>
                                    <tr>
                                ...
                                </table>) : <TextPlaceHolder paragraph={4} className="table-two"/>}
                            </div>
